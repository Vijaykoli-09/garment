    ...
    const bodyRowsHtml = sortedGroupRows
      .map((g, idx) => {
        ...
        for (let line = 0; line < 4; line++) {
          ...
          const labelText = line === 0 ? "Box" : line === 1 ? "Pcs" : line === 2 ? "Rate" : "Amount";
          const labelCell = `<td style="border:1px solid #000;padding:4px;text-align:left">${labelText}</td>`;
                ...
                <tbody>
                  {sortedGroupRows.map((g, idx) => {
                    ...
                    return (
                      <React.Fragment key={`${g.artSerial}-${g.shadeName}-${idx}`}>
                        {[0, 1, 2, 3].map((line) => {
                          const label = line === 0 ? "Box" : line === 1 ? "Pcs" : line === 2 ? "Rate" : "Amount";

                          return (
                            <tr key={`${g.artSerial}-${g.shadeName}-${idx}-${line}`} className={bgClass}>
                              {line === 0 && (
                                <>
                              ...
                              <td className="px-2 py-1 border border-gray-400 text-left">{label}</td>
