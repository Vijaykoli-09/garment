...
// ✅ ADDED: pending (LIFO-based)
type DisplayRowWithDaysPending = DisplayRowWithDays & {
  pending: number;
...
const AccountStatement: React.FC = () => {
  ...
  // Drill-down
  const [selectedTx, setSelectedTx] = useState<DisplayRowWithDaysPending | null>(null);
  const detailRef = useRef<HTMLDivElement | null>(null);
  ...
  // ✅ ADDED: Party master opening (DR=+, CR=-)
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      payments.forEach((p) => {
        const bName =
          (p.brokerName || "").trim() || (p.partyName ? getBrokerFromPartyName(p.partyName) : "");
      ...
      if (showOpening) {
        // ✅ CHANGED: Party opening if party selected, else Agent opening (broker only)
        let openingBal = 0;
  ...
  // ✅ LIFO pending calculation
  const rowsWithPending: DisplayRowWithDaysPending[] = useMemo(() => {
    if (!rowsWithDays.length) return [];

    // pending side derived from closing position
    const pendingSide: "DEBIT" | "CREDIT" = closingBalanceAll >= 0 ? "DEBIT" : "CREDIT";

    // exclude opening from allocation (opening ka aging unknown)
    const core = rowsWithDays.filter((r) => r.type !== "Opening");

    // stack of charges (LIFO)
    type StackItem = { srNo: number; remaining: number };
    const stack: StackItem[] = [];
    const remainingMap = new Map<number, number>(); // srNo -> remaining

    const chargeAmt = (r: DisplayRowWithDays) => (pendingSide === "DEBIT" ? r.debit : r.credit);
    const settleAmt = (r: DisplayRowWithDays) => (pendingSide === "DEBIT" ? r.credit : r.debit);
    ...
    return rowsWithDays.map((r) => {
      if (r.type === "Opening") return { ...r, pending: 0, pendingSide };
      const ch = toNum(chargeAmt(r));
      const pending = ch > 0 ? toNum(remainingMap.get(r.srNo) ?? 0) : 0;
      return { ...r, pending, pendingSide };
    });
  }, [rowsWithDays, closingBalanceAll]);

  // ---------- Filter ----------
  const filteredRows: DisplayRowWithDaysPending[] = useMemo(() => {
    if (transactionFilter === "all") return rowsWithPending;
    return rowsWithPending.filter((r) => r.type === transactionFilter);
  }, [rowsWithPending, transactionFilter]);
  ...
  // ---------- Overdue (✅ now based on LIFO pending > 0) ----------
  const overdueRowsAll: OverdueAlertRow[] = useMemo(() => {
    const list = rowsWithPending
      .filter((r) => r.type !== "Opening" && r.pending > 0 && r.days >= OVERDUE_DAYS)
      .map((r) => {
        const b =
          (r.brokerName || "").trim() || (r.partyName ? getBrokerFromPartyName(r.partyName) : "") || "-";
    ...
    list.sort((a, b) => b.days - a.days);
    return list;
  }, [rowsWithPending, getBrokerFromPartyName]);
  ...
  useEffect(() => {
    ...
    const html = `
      <div style="text-align:left; font-size: 13px;">
        <div style="margin-bottom:8px;"><b>Overdue Pending Entries (LIFO) (≥ ${OVERDUE_DAYS} Days)</b></div>
        <table style="width:100%; border-collapse: collapse;" border="1" cellpadding="6">
          <thead>
            <tr style="background:#fee2e2;">
              <th>Party</th>
  ...
  const openDetails = (tx: DisplayRowWithDaysPending) => {
    if (tx.type === "Opening") return;
    setSelectedTx(tx);
    window.setTimeout(() => {
      detailRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  // PRINT (✅ added pending column + overdue logic on pending)
  const handlePrintReport = () => {
    if (typeof window === "undefined" || typeof document === "undefined") return;
    if (!filteredRows.length) {
      alert("No transactions to print");
    ...
    const rowsHtml = filteredRows
      .map((r) => {
        const overdue = r.type !== "Opening" && r.pending > 0 && r.days >= OVERDUE_DAYS;
        const cls = overdue
          ? "row-overdue"
          : r.type === "Payment"
            ? "row-payment"
        ...
        return `
          <tr class="${cls}">
            ...
            <td class="text-right">${fmtNumber(r.credit)}</td>
            <td class="text-right">${fmtNumber(r.balance)}</td>
            <td class="text-right">${fmtNumber(r.pending)}</td>
            <td>${typeLabel(r.type)}</td>
            <td class="text-right ${overdue ? "days-overdue" : ""}">${r.days}</td>
          </tr>
        `;
...
<html>
  ...
  <body>
    ...
    <div class="info">
      ...
      <div style="margin-top:6px; color:#444;">
        Overdue rule: Pending > 0 (LIFO) AND Days ≥ ${OVERDUE_DAYS} highlighted in red.
      </div>
    </div>
            ...
            >
              <div className="flex items-center justify-between p-3 border-b">
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Pending Side (LIFO): <b>{rowsWithPending[0]?.pendingSide || "DEBIT"}</b>
                  </div>
              ...
              <div className="p-2 overflow-auto" style={{ height: fullScreen ? "calc(100vh - 72px)" : "78vh" }}>
                {overdueRowsAll.length > 0 && (
                  <div className="mb-2 border border-red-300 bg-red-50 rounded p-2 text-xs">
                    <div className="font-semibold text-red-700">
                      Overdue Pending Entries (LIFO) (≥ {OVERDUE_DAYS} Days): {overdueRowsAll.length}
                    </div>
                  </div>
                )}
                ...
                {selectedTx && selectedTxDetails && (
                  <div className="mb-3 border rounded p-3 bg-gray-50">
                    ...
                    <div className="mt-2 grid grid-cols-12 gap-2 text-xs">
                      ...
                      <div className="col-span-12 lg:col-span-3 text-gray-600">Pending (LIFO)</div>
                      <div className="col-span-12 lg:col-span-9 text-gray-900">{fmtNumber(selectedTx.pending)}</div>
                ...
                <div className="min-w-max">
                  <table className="w-full table-auto text-sm border-collapse">
                    ...
                    <tbody>
                      ...
                      {filteredRows.length === 0 && (
                        <tr>
                          <td colSpan={11} className="px-2 py-3 border text-center text-gray-500">
                            No transactions found.
                          </td>
                        </tr>
                      )}
                            ...
                            <div className="w-full bg-white border-t">
                              <div className="p-3">
                                ...
                                <div className="mt-3 text-xs text-gray-600">
                                  Red row = Pending (LIFO) & {OVERDUE_DAYS}+ days.
                                </div>
                              </div>
                            </div>
                          </td>
                ...
                {filteredRows.length > 0 && (
                  <div className="mt-2 text-xs text-gray-700 flex items-center gap-3">
                    <span className="inline-flex items-center gap-2">
                      <span className="w-4 h-3 bg-red-100 border inline-block" />
                      Pending (LIFO) & {OVERDUE_DAYS}+ Days
                    </span>
                  </div>
                )}
              </div>
