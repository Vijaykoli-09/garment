...
interface DispatchReturnChallan {
  id: number;
  challanNo: string;
  date?: string;
  dated?: string;
...
const AccountStatement: React.FC = () => {
  ...
  const [dispatchReturnChallans, setDispatchReturnChallans] = useState<DispatchReturnChallan[]>([]);
      ...
      try {
        const [partyRaw, agentRaw, dcRaw, drcRaw, odcRaw, poRaw, peRaw, prRaw, payRaw, jobOutRaw, jobInRaw] =
          await Promise.all([
            ...
            safeGet<any[]>("/dispatch-return-challan"),
        ...
        setDispatchReturnChallans(
          (Array.isArray(drcRaw) ? drcRaw : []).map((drc: any) => ({
            id: drc.id,
            challanNo: String(drc.challanNo ?? ""),
            date: drc.date || drc.dated || "",
            dated: drc.dated,
            partyName: drc.partyName || "",
            brokerName: (drc.brokerName || drc.agentName || "").trim(),
            netAmt: drc.netAmt,
          })),
        );
  ...
  const getAmounts = useCallback((source: TxType, amount: number, discount?: number) => {
    ...
    if (source === "DispatchReturn") return { debit: 0, credit: amt, discount: 0 };
  ...
  const brokerInfos: BrokerInfo[] = useMemo(() => {
    ...
    dispatchReturnChallans.forEach((d) => add(getBrokerNameForDoc(d), d.partyName));
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      // ✅ Dispatch Return Challan (CREDIT)
      dispatchReturnChallans.forEach((drc) => {
        const bName = getBrokerNameForDoc(drc);
        if (!brokerOk(bName)) return;
        if (!partyOk(drc.partyName)) return;

        docs.push({
          source: "DispatchReturn",
          id: drc.id,
          date: (drc.date || drc.dated || "") || fromDate,
          number: drc.challanNo,
          partyName: drc.partyName,
          brokerName: bName,
          amount: toNum(drc.netAmt),
          docKey: makeDocKey("DispatchReturn", drc.id),
        });
      });
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-xs">
                    ...
                    <select value={transactionFilter} onChange={(e) => setTransactionFilter(e.target.value as any)} className="border rounded px-2 py-1 text-xs">
                      ...
                      <option value="DispatchReturn">Dispatch Return</option>
