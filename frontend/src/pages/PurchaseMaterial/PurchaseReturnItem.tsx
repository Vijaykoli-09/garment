...
const handlePartyChange = async (partyId: string) => {
  ...
  try {
    const res = await api.get(`/purchase/return-item/template/${partyId}`);
...
const buildPayload = () => ({
  ...
  try {
    ...
    await (selectedReturnId
      ? api.put(`/purchase/return-item/${selectedReturnId}`, payload)
      : api.post("/purchase/return-item", payload));
  ...
  const handleDelete = async () => {
    ...
    if (confirm.isConfirmed) {
      try {
        await api.delete(`/purchase/return-item/${selectedReturnId}`);
  ...
  const fetchReturns = async () => {
    try {
      const res = await api.get("/purchase/return-item");
      setReturns(res.data);
    ...
    <Dashboard>
      <div className="bg-gray-100 p-6 min-h-screen">
        <div className="bg-white shadow-md p-6 rounded-lg">
          <h2 className="mb-4 font-bold text-2xl text-center"> Material Purchase Return</h2>
