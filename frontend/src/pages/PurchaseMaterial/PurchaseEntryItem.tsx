...
const PurchaseEntry: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const [partyRes, matRes, shadeRes, groupRes] =
          await Promise.all([
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await (entryId
        ? api.put(`/purchase/entry-item/${entryId}`, payload)
        : api.post("/purchase/entry-item", payload));
  ...
  //     let id = entryId;
  //     if (id) {
  //       await api.put(`/purchase/entry-item/${id}`, payload);
  //     } else {
  //       const res = await api.post("/purchase/entry-item", payload);
  //       id = res.data?.id;
  //       setEntryId(id);
  //     }

  //     await api.post(`/purchase/entry-item/${id}/issue`);
  ...
  const fetchEntries = async () => {
    try {
      const res = await api.get("/purchase/entry-item");
  ...
  const handleEditEntry = async (entrySummary: any) => {
    try {
      const res = await api.get(`/purchase/entry-item/${entrySummary.id}`);
  ...
  const handleDeleteEntry = async (id: number) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await api.delete(`/purchase/entry-item/${id}`);
