...
interface Customer {
  ...
  email:           string;
  phone:           string;
  customerType:    string;   // empty until admin sets it during approval
...
// Prefill data passed to PartyCreation when "Create Party" is clicked
export interface PartyPrefill {
  partyName:  string;
  mobileNo:   string;
  gstNo:      string;
  address:    string;
  customerId: number;   // so we can auto-link after party creation
}

interface Props {
  // Called when admin clicks "Create Party →" — parent switches to PartyCreation
  // and passes the prefill data
  onCreateParty: (prefill: PartyPrefill) => void;
}

// ── Customer type options (set by admin during approval) ─────────────────────
...
const CustomerRequests: React.FC<Props> = ({ onCreateParty }) => {
  ...
  const [customerType, setCustomerType]   = useState<string>("");   // ← admin sets this
  ...
  }, [customers, search, selectedDate, selectedStatus]);
  ...
  const openApproveModal = (customer: Customer) => {
    ...
    setCustomerType("");          // admin must pick type
    ...
    try {
      if (modalMode === "approve") {
        ...
        setCustomers((prev) =>
          prev.map((c) =>
            c.id === activeCustomer.id
              ? { ...c, status: "APPROVED", customerType, creditEnabled, creditLimit: creditEnabled ? Number(creditLimit) : 0, advanceOption }
              : c
          )
        );
      } else {
        ...
        setCustomers((prev) =>
          prev.map((c) =>
            c.id === activeCustomer.id
              ? { ...c, status: formStatus, customerType, creditEnabled: formStatus === "APPROVED" ? creditEnabled : false, creditLimit: formStatus === "APPROVED" && creditEnabled ? Number(creditLimit) : 0, advanceOption: formStatus === "APPROVED" ? advanceOption : false }
              : c
          )
        );
      }
  ...
  const handleLinkParty = async (customerId: number) => {
    if (!selectedPartyId) { setLinkError("Please select a party."); return; }
    setLinkSaving(true);
    setLinkError("");
    try {
      ...
      setCustomers((prev) =>
        prev.map((c) => c.id === customerId ? { ...c, partyId: Number(selectedPartyId) } : c)
      );
      closeLinkParty();
    } catch {
  ...
  // ── Create Party → navigate to PartyCreation with prefill ────────
  const handleCreateParty = (customer: Customer) => {
    onCreateParty({
  ...
  return (
    <Dashboard>
      <div style={page}>
        ...
        {/* Filters — no type filter since pending customers have no type yet */}
        <div style={filterRow}>
          <input
          ...
          {(search || selectedDate || selectedStatus) && (
            <button style={clearBtn}
              onClick={() => { setSearch(""); setSelectedDate(""); setSelectedStatus(""); }}>
              ✕ Clear
            </button>
          )}
        </div>
              ...
              <div key={cust.id} style={card}>
                ...
                <div style={cardTop}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {/* Show type only if set (approved customers) */}
                    {cust.customerType
                      ? <div style={cardType}>{cust.customerType.replace("_", " ")}</div>
                      : <div style={{ ...cardType, color: "#f59e0b" }}>Type not set yet</div>
                    }
                  </div>
                ...
                <div style={detailGrid}>
                  {cust.email && <Detail label="📧" value={cust.email} />}
                ...
                {cust.status === "APPROVED" && (
                  <div style={paymentInfo}>
                    <div style={paymentRow}>
                      <span style={payLabel}>Customer Type</span>
                      <span style={payVal("#2563eb")}>{cust.customerType?.replace("_", " ") || "—"}</span>
                    </div>
                    <div style={paymentRow}>
                ...
                <div style={cardActions}>
                  ...
                  {/* Approved: show both Create Party and Link Party */}
                  {cust.status === "APPROVED" && (
                    <>
                      {/* Create Party — opens PartyCreation with prefill */}
                      {!cust.partyId && (
                        <button style={btnCreateParty} onClick={() => handleCreateParty(cust)}>
                          🏷️ Create Party →
                        </button>
                      )}
                      ...
                      <button
                        ...
                        onClick={() => linkingCustomerId === cust.id ? closeLinkParty() : openLinkParty(cust.id)}
                      >
                        {cust.partyId
                          ? "🔗 Relink"
        ...
        {modalOpen && activeCustomer && (
          <div style={overlay} onClick={closeModal}>
            <div style={modal} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={section}>
                  ...
                  <div style={toggleRow}>
                    {(["APPROVED", "REJECTED"] as const).map((st) => (
                      <button
                        key={st}
                        style={statusToggleBtn(formStatus === st, st)}
                        onClick={() => {
                          setFormStatus(st);
                          setModalError("");
                          if (st === "REJECTED") {
              ...
              {formStatus === "APPROVED" && (
                <div style={section}>
                  ...
                  <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 10px" }}>
                    {modalMode === "approve"
                      ? "Call the customer and confirm their business type before selecting."
                      : "Update the customer type if needed."
                    }
                  </p>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {CUSTOMER_TYPES.map((t) => (
                      <button
                        key={t.value}
                        onClick={() => { setCustomerType(t.value); setModalError(""); }}
                        style={typeSelectBtn(customerType === t.value)}
                      >
                        {t.label}
                      </button>
                    ))}
                  </div>
                  {!customerType && modalError && (
                    <p style={errText}>{modalError}</p>
                  )}
                </div>
              )}

              {/* Payment Config (approved only) */}
              {formStatus === "APPROVED" && (
                <>
                  {/* Credit Enable */}
                  <div style={section}>
                    ...
                    <div style={section}>
                      ...
                      <div style={presetRow}>
                        {[50000, 100000, 250000, 500000, 1000000].map((amt) => (
                          <button key={amt} style={presetBtn(Number(creditLimit) === amt)}
                      ...
                      <div style={amountRow}>
                        <button style={amtBtn}
                          onClick={() => setCreditLimit((prev) => Math.max(0, Number(prev) - 10000).toString())}>−</button>
                        <div style={amountInputWrap}>
                          <span style={rupeeSign}>₹</span>
                          <input type="number" value={creditLimit}
                            onChange={(e) => setCreditLimit(e.target.value)}
                            style={amountInput} min={0} step={10000} />
                        </div>
                        <button style={amtBtn}
                          onClick={() => setCreditLimit((prev) => (Number(prev) + 10000).toString())}>+</button>
                      </div>

                      {/* 30/70 Advance Option */}
                      <div style={{ marginTop: 16 }}>
                        <label style={checkRow}>
                          <div style={toggleSw(advanceOption)} onClick={() => setAdvanceOption(!advanceOption)}>
              ...
              {/* Error (non-credit-related) */}
              {modalError && !(!customerType && modalError) && (
                <p style={{ ...errText, margin: "0 20px 12px" }}>{modalError}</p>
              )}

              {/* Footer */}
              <div style={modalFooter}>
                <button style={cancelBtn} onClick={closeModal} disabled={saving}>Cancel</button>
...
const refreshBtn: React.CSSProperties   = { padding: "8px 16px", background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, cursor: "pointer", fontSize: 13, color: "#475569", fontWeight: 600 };
const errorBanner: React.CSSProperties  = { background: "#fee2e2", color: "#dc2626", padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 13 };

const chipRow: React.CSSProperties = { display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" };
const summaryChip = (color: string): React.CSSProperties => ({ display: "flex", alignItems: "center", gap: 8, background: "#fff", border: `1px solid ${color}22`, borderLeft: `3px solid ${color}`, borderRadius: 8, padding: "10px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" });
const chipNum = (color: string): React.CSSProperties => ({ fontSize: 20, fontWeight: 800, color });
...
const filterInput: React.CSSProperties = { padding: "8px 12px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 13, minWidth: 150, background: "#fff", color: "#374151" };
const clearBtn: React.CSSProperties    = { padding: "8px 14px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, cursor: "pointer", fontSize: 13, color: "#dc2626", fontWeight: 600 };

const cardGrid: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 };
const card: React.CSSProperties     = { background: "#fff", borderRadius: 12, padding: 16, border: "1px solid #e2e8f0", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" };
const cardTop: React.CSSProperties  = { display: "flex", alignItems: "center", gap: 12, marginBottom: 12 };
const avatar: React.CSSProperties   = { width: 40, height: 40, borderRadius: "50%", background: "#e0e7ff", color: "#4f46e5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 800, flexShrink: 0 };
...
const statusBadgeStyle = (status: string): React.CSSProperties => {
  ...
  return { padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap", ...(map[status] ?? {}) };
};

const paymentInfo: React.CSSProperties  = { background: "#f8fafc", borderRadius: 8, padding: "10px 12px", border: "1px solid #e2e8f0", marginBottom: 12 };
...
const btnApprove: React.CSSProperties   = { flex: 1, padding: "8px", background: "#10b981", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13 };
const btnReject: React.CSSProperties    = { flex: 1, padding: "8px", background: "#ef4444", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13 };
const btnEdit: React.CSSProperties      = { flex: 1, padding: "8px", background: "#f1f5f9", color: "#475569", border: "1px solid #e2e8f0", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13 };
const btnLinkParty: React.CSSProperties = { flex: 1, padding: "8px", background: "#eff6ff", color: "#2563eb", border: "1px solid #bfdbfe", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13 };
const btnLinked: React.CSSProperties    = { flex: 1, padding: "8px", background: "#f0fdf4", color: "#15803d", border: "1px solid #bbf7d0", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13 };
// New — prominent orange button for creating party
const btnCreateParty: React.CSSProperties = { flex: 1, padding: "8px", background: "#fff7ed", color: "#c2410c", border: "1px solid #fed7aa", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13 };
const linkPartyPanel: React.CSSProperties = { background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: "12px", marginBottom: 10 };

const overlay: React.CSSProperties     = { position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 };
const modal: React.CSSProperties       = { background: "#fff", borderRadius: 14, width: 480, maxWidth: "95vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" };
const modalHeader: React.CSSProperties = { display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "20px 20px 14px", borderBottom: "1px solid #f1f5f9" };
...
const closeBtn: React.CSSProperties    = { background: "none", border: "none", fontSize: 18, cursor: "pointer", color: "#94a3b8", lineHeight: 1, padding: 4 };
const section: React.CSSProperties     = { padding: "14px 20px", borderBottom: "1px solid #f1f5f9" };
const sectionLabel: React.CSSProperties = { fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase" as const, letterSpacing: "0.8px", marginBottom: 10 };
...
const statusToggleBtn = (active: boolean, status: string): React.CSSProperties => {
  ...
  return { flex: 1, padding: "10px 0", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13, background: active ? colors.activeBg : colors.bg, color: active ? colors.activeColor : colors.color, border: `1px solid ${colors.border}` };
};

// Customer type selection buttons
const typeSelectBtn = (active: boolean): React.CSSProperties => ({
...
const toggleSw = (on: boolean): React.CSSProperties => ({ width: 44, height: 24, borderRadius: 12, cursor: "pointer", flexShrink: 0, background: on ? "#10b981" : "#e2e8f0", position: "relative", transition: "background 0.2s" });
const toggleThumb = (on: boolean): React.CSSProperties => ({ position: "absolute", top: 2, left: on ? 22 : 2, width: 20, height: 20, borderRadius: "50%", background: "#fff", boxShadow: "0 1px 4px rgba(0,0,0,0.2)", transition: "left 0.2s" });
...
const presetBtn = (active: boolean): React.CSSProperties => ({ padding: "5px 12px", borderRadius: 20, fontSize: 12, cursor: "pointer", fontWeight: 600, background: active ? "#2563eb" : "#f1f5f9", color: active ? "#fff" : "#475569", border: active ? "1px solid #2563eb" : "1px solid #e2e8f0" });

const amountRow: React.CSSProperties   = { display: "flex", alignItems: "center", gap: 10 };
const amtBtn: React.CSSProperties      = { width: 36, height: 36, borderRadius: 8, border: "1px solid #e2e8f0", background: "#f8fafc", cursor: "pointer", fontSize: 20, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", justifyContent: "center" };
const amountInputWrap: React.CSSProperties = { flex: 1, display: "flex", alignItems: "center", border: "1.5px solid #e2e8f0", borderRadius: 8, overflow: "hidden", background: "#fff" };
const rupeeSign: React.CSSProperties   = { padding: "0 10px", fontSize: 15, color: "#64748b", fontWeight: 600, borderRight: "1px solid #e2e8f0", background: "#f8fafc" };
const amountInput: React.CSSProperties = { flex: 1, padding: "9px 10px", border: "none", outline: "none", fontSize: 15, fontWeight: 700, color: "#0f172a" };

const errText: React.CSSProperties     = { color: "#dc2626", fontSize: 12, marginTop: 8, marginBottom: 0 };
const modalFooter: React.CSSProperties = { padding: "16px 20px", display: "flex", gap: 10, justifyContent: "flex-end" };
const cancelBtn: React.CSSProperties   = { padding: "10px 20px", background: "#f1f5f9", color: "#475569", border: "1px solid #e2e8f0", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 13 };
const saveBtn = (status: string): React.CSSProperties => ({ padding: "10px 20px", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13, background: status === "APPROVED" ? "#10b981" : "#ef4444" });
...
const spinner: React.CSSProperties     = { width: 36, height: 36, border: "3px solid #e2e8f0", borderTop: "3px solid #6366f1", borderRadius: "50%", animation: "spin 0.8s linear infinite" };
const emptyState: React.CSSProperties  = { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 60, color: "#94a3b8" };
