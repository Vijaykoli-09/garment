...
const SaleOrderPendencyArtSizeWise: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Sale Orders list (for availableArts)
      try {
  ...
  const showReport = async () => {
    ...
    const selectedShadeNameSet = new Set(
      selectedShades.map((s) => norm(s.name)),
    );
    ...
    try {
      ...
      const { data } = await api.get<SaleOrderPendencyRowDTO[]>(
        "/sale-orders/pendency",
        {
          params: {
            fromDate,
            toDate,
      ...
      let baseRows: SORow[] = list.map((r, i) => {
        const destId =
          destinations.find(
            (d) => norm(d.name) === norm(r.destination || ""),
          )?.id ?? 0;
        ...
        const shadeVal = String(r.shade || "").trim();
        const sizeUpper = String(r.size || "").trim().toUpperCase();
      ...
      // ✅ sirf pending != 0 (0 rows skip), negative allowed
  ...
  const groupedRows = useMemo<GroupedRow[]>(() => {
    ...
    rows.forEach((r) => {
      ...
      const cell = g.perSize[r.size] || {
        opening: 0,
        receipt: 0,
        dispatch: 0,
        pending: 0,
      };
      cell.opening += r.opening || 0;
      cell.receipt += r.receipt || 0;
      cell.dispatch += r.dispatch || 0;
      cell.pending += r.pending || 0;
      g.perSize[r.size] = cell;
  ...
  // ---------- fulfill helpers ----------
  const buildPayloadForGroup = (g: GroupedRow): SaleOrderPendencyRowDTO[] => {
    const payloadRows: SaleOrderPendencyRowDTO[] = [];
  ...
  const handleFulfillGroup = async (g: GroupedRow) => {
    ...
    const confirm = await Swal.fire({
      ...
      html: `
        <div style="text-align:left;font-size:13px">
          ...
          <div style="margin-top:6px"><b>Pending Boxes:</b> ${fmt(
            g.pendingTotal,
          )}</div>
        </div>
      `,
    ...
    try {
      ...
      Swal.fire(
        "Error",
        "Could not fulfill this row. Please try again.",
        "error",
      );
    } finally {
  ...
  const handleFulfillAll = async () => {
    const allPayloadRows = groupedRows.flatMap((g) =>
      buildPayloadForGroup(g),
    );
    if (!allPayloadRows.length) {
      return Swal.fire("Info", "No pending quantity to fulfill.", "info");
    }

    const totalPending = groupedRows.reduce(
      (sum, g) => sum + (g.pendingTotal || 0),
      0,
    );
    ...
    const confirm = await Swal.fire({
      ...
      html: `
        <div style="text-align:left;font-size:13px">
          ...
          <div><b>Total Lines (party/art/size):</b> ${
            allPayloadRows.length
          }</div>
          <div style="margin-top:6px"><b>Total Pending Boxes:</b> ${fmt(
            totalPending,
          )}</div>
        </div>
      `,
              ...
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Broker (Agent) → Destination → Party → Art → Size → Shade
                    wise pending quantity.
                  </p>
                </div>
              </div>
