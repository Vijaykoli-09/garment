...
type SORow = {
  ...
  // old order/challan ref
  orderRef?: string;

  // NEW: unique source key so same order no. in future won't merge
  sourceKey?: string;
  sourceDate?: string;
...
// NEW: stronger unique source key extractor
const getSourceKey = (x: any, parent?: any) => {
  const direct = String(
    x?.sourceKey ??
      x?.sourceRowKey ??
      x?.sourceRef ??
      ...
      parent?.sourceRowKey ??
      parent?.sourceRef ??
      parent?.sourceUniqueKey ??
      parent?.saleOrderRowId ??
      parent?.saleOrderItemId ??
      parent?.saleOrderDetailId ??
      parent?.detailId ??
      parent?.detailRowId ??
      parent?.rowId ??
      parent?.lineId ??
  ...
  const ref = getOrderRef(x) || getOrderRef(parent);
  const dated = String(x?.dated ?? x?.date ?? parent?.dated ?? parent?.date ?? "")
    .slice(0, 10)
    .trim();
  const partyId = String(x?.partyId ?? parent?.partyId ?? "").trim();
  const partyName = String(x?.partyName ?? parent?.partyName ?? "").trim();
  const artNo = String(x?.artNo ?? parent?.artNo ?? "").trim();
  const size = String(x?.size ?? x?.sizeName ?? "").trim();
  const shade = String(
    x?.shade ??
      x?.shadeName ??
      x?.shadeCode ??
  ...
  const rowNo = String(
    x?.lineNo ?? x?.rowNo ?? x?.srNo ?? x?.serialNo ?? x?.detailNo ?? ""
  ).trim();

  const parts = [partyId || partyName, ref, dated, artNo, size, shade, rowNo].filter(Boolean);
  return parts.join("|");
};
...
const SaleOrderPendencyArtSizeWise: React.FC = () => {
    ...
    (async () => {
      try {
        ...
        const stationNames = mappedParties
          ...
          .filter(Boolean);
        const uniqueStations = Array.from(new Set(stationNames));
        setDestinations(uniqueStations.map((name, idx) => ({ id: idx + 1, name })));
  ...
  const availableDestinations = useMemo(() => {
    ...
    const selBrokerNames = new Set(
      brokers.filter((b) => selBrokerIds.includes(b.id)).map((b) => norm(b.name))
    );
  ...
  const availableSizes = useMemo(() => {
    if (selDestIds.length === 0 || selPartyIds.length === 0 || selArtIds.length === 0)
      return [];
    return allSizes;
  }, [selDestIds, selPartyIds, selArtIds, allSizes]);
  ...
  const showReport = async () => {
    ...
    try {
      ...
      let baseRows: SORow[] = list.map((r: any, i: number) => {
        ...
        const orderRef = getOrderRef(r);
        const sourceKey = getSourceKey(r);
        const sourceDate = String(r.dated ?? r.date ?? "").slice(0, 10);
        ...
        return {
          id: Number(r.id ?? r.rowId ?? r.lineId ?? i + 1),
          destinationId: destId,
          partyId: Number(party.id),
          artId: masterArt?.id ?? 0,
          size: String(r.size || ""),
      ...
      // ---------- Dispatch Challan ----------
      const dispatchMapBySource = new Map<string, number>(); // party|art|source|shade|size
      const dispatchMapBySourceNoShade = new Map<string, number>(); // party|art|source|size
      const dispatchMap = new Map<string, number>(); // party|art|order|shade|size
      const dispatchMapNoShade = new Map<string, number>(); // party|art|order|size
      const dispatchMapNoOrder = new Map<string, number>(); // party|art|shade|size
      const dispatchMapNoOrderNoShade = new Map<string, number>(); // party|art|size
      ...
      try {
        ...
        challans.forEach((ch: any) => {
          ...
          rowsDc.forEach((rr: any) => {
            ...
            const shade = String(rr.shade ?? rr.shadeName ?? rr.shadeCode ?? "").trim();
            const rowOrderRef = getOrderRef(rr) || docOrderRef;
            const rowSourceKey = getSourceKey(rr, ch);
            ...
            if (rowSourceKey) {
              const ks1 = `${norm(pName)}|${norm(artNo)}|${norm(rowSourceKey)}|${norm(shade)}|${norm(size)}`;
              dispatchMapBySource.set(ks1, (dispatchMapBySource.get(ks1) || 0) + box);

              const ks2 = `${norm(pName)}|${norm(artNo)}|${norm(rowSourceKey)}|${norm(size)}`;
              dispatchMapBySourceNoShade.set(ks2, (dispatchMapBySourceNoShade.get(ks2) || 0) + box);
            }
        ...
        baseRows = baseRows.map((row) => {
          ...
          const sizeKey = norm(row.size);
          const orderKey = norm(row.orderRef || "");
          const sourceKeyNorm = norm(row.sourceKey || "");

          let disp = 0;

          if (row.sourceKey) {
            const ks1 = `${partyKey}|${artKey}|${sourceKeyNorm}|${shadeKey}|${sizeKey}`;
            const ks2 = `${partyKey}|${artKey}|${sourceKeyNorm}|${sizeKey}`;
            disp = dispatchMapBySource.get(ks1) ?? dispatchMapBySourceNoShade.get(ks2) ?? 0;
          }

          if (!disp && row.orderRef) {
            const k1 = `${partyKey}|${artKey}|${orderKey}|${shadeKey}|${sizeKey}`;
            const k2 = `${partyKey}|${artKey}|${orderKey}|${sizeKey}`;
            disp = dispatchMap.get(k1) ?? dispatchMapNoShade.get(k2) ?? 0;
          }
      ...
      // ---------- Order Settle / Fulfill ----------
      try {
        const { data: osData } = await api.get<any[]>("/order-settles");
        const settles = Array.isArray(osData) ? osData : [];

        const settleMapBySource = new Map<string, number>(); // party|art|source|shade|size
        const settleMapBySourceNoShade = new Map<string, number>(); // party|art|source|size
        const settleMap = new Map<string, number>(); // party|art|order|shade|size
        const settleMapNoShade = new Map<string, number>(); // party|art|order|size
        const settleMapNoOrder = new Map<string, number>(); // party|art|shade|size
        const settleMapNoOrderNoShade = new Map<string, number>(); // party|art|size
        ...
        settles.forEach((doc: any) => {
          ...
          rowsOs.forEach((rr: any) => {
            ...
            const shade = String(rr.shade ?? rr.shadeName ?? rr.shadeCode ?? "").trim();
            const rowOrderRef = getOrderRef(rr) || docOrderRef;
            const rowSourceKey = getSourceKey(rr, doc);
            ...
            const dets = Array.isArray(rr.sizeDetails) ? rr.sizeDetails : [];
            dets.forEach((sd: any) => {
              const size = String(sd.sizeName || sd.size || "").trim();
              if (!size) return;
              if (!selSizes.map((s) => s.toUpperCase()).includes(size.toUpperCase())) return;
              ...
              if (rowSourceKey) {
                const ks1 = `${norm(pName)}|${norm(artNo)}|${norm(rowSourceKey)}|${norm(shade)}|${norm(size)}`;
                settleMapBySource.set(ks1, (settleMapBySource.get(ks1) || 0) + qty);

                const ks2 = `${norm(pName)}|${norm(artNo)}|${norm(rowSourceKey)}|${norm(size)}`;
                settleMapBySourceNoShade.set(ks2, (settleMapBySourceNoShade.get(ks2) || 0) + qty);
              }
        ...
        baseRows = baseRows.map((row) => {
          ...
          if (row.sourceKey) {
            const ks1 = `${partyKey}|${artKey}|${sourceKeyNorm}|${shadeKey}|${sizeKey}`;
            const ks2 = `${partyKey}|${artKey}|${sourceKeyNorm}|${sizeKey}`;
            settled = settleMapBySource.get(ks1) ?? settleMapBySourceNoShade.get(ks2) ?? 0;
          }

          if (!settled && row.orderRef) {
            const k1 = `${partyKey}|${artKey}|${orderKey}|${shadeKey}|${sizeKey}`;
            const k2 = `${partyKey}|${artKey}|${orderKey}|${sizeKey}`;
            settled = settleMap.get(k1) ?? settleMapNoShade.get(k2) ?? 0;
          }
  ...
  // group by sourceKey first, fallback orderRef
  const groupedRows = useMemo<GroupedRow[]>(() => {
    const map = new Map<string, GroupedRow>();

    rows.forEach((r) => {
      const internalSource = norm(r.sourceKey || r.orderRef || "");
    ...
    return arr.sort((a, b) => {
      ...
      const p = a.partyName.localeCompare(b.partyName);
      if (p !== 0) return p;
      const s = (a.sourceDate || "").localeCompare(b.sourceDate || "");
      if (s !== 0) return s;
      const src = (a.sourceKey || a.orderRef || "").localeCompare(b.sourceKey || b.orderRef || "");
      if (src !== 0) return src;
      const aCmp = a.artNo.localeCompare(b.artNo);
      if (aCmp !== 0) return aCmp;
      return (a.shade || "").localeCompare(b.shade || "");
  ...
  const buildPayloadForGroup = (g: GroupedRow, fulfillBatchId: string) => {
    const payloadRows: any[] = [];
    ...
    sizeColumns.forEach((s) => {
      ...
      payloadRows.push({
        ...
        orderRef: g.orderRef || "",
        sourceOrderRef: g.orderRef || "",

        // NEW
        sourceKey: g.sourceKey || g.orderRef || "",
        sourceDate: g.sourceDate || "",
        fulfillBatchId,
        forceCreate: true,
        separateBySourceKey: true,
        lineCreateKey: `${g.sourceKey || g.orderRef || g.key}|${s}|${fulfillBatchId}`,
  ...
  const handleFulfillGroup = async (g: GroupedRow) => {
    const fulfillBatchId = makeUniqueId("ROWFUL");
    const payloadRows = buildPayloadForGroup(g, fulfillBatchId);
  ...
  const handleFulfillAll = async () => {
    const fulfillBatchId = makeUniqueId("ALLFUL");
    const allPayloadRows = groupedRows.flatMap((g) => buildPayloadForGroup(g, fulfillBatchId));
